...
my %people = (
	"ryan"=>\%ryan_favorites,
	"matt"=>\%matt_favorites,
	"jack"=>{"food"=>["ice cream"], "basketball team"=>["blackhawks"]},
...
sub addProperty {
	...
	print "What is ${person}'s favorite $property? (Comma Separated)\n";
...
sub changeProperty {
	...
	print "Enter the new value(s) for the properties $property: (Comma Separated)";
	chomp (my $value = <STDIN>);
    my @arr = split($value, ",");
