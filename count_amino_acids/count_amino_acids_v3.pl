...
{
    if($amino_acids{$key} == 0){
        push(@dif_acids, $key);
        delete($amino_acids{$key});
    }
    else{
...
if(scalar @dif_acids != 0){
    ...
    print "\nNumber of different amino acids: $odd_occ\n";
    my $response = "";
    if($#dif_acids <= 1){
...
foreach my $index (sort { $amino_acids{$b} <=> $amino_acids{$a} } keys(%amino_acids))
{
    print "$index: $amino_acids{$index}\n";
}